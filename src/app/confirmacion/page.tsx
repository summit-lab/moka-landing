...
type SearchParams = Promise<{ assigned_to?: string; event_type_name?: string }>

export default async function ConfirmacionPage({ searchParams }: { searchParams: SearchParams }) {
  const params = await searchParams
  const name = params.assigned_to?.split(' ')[0] ?? null
  ...
  return (
    <>
      ...
      >
        <div className="absolute inset-0 pointer-events-none" style={{ background: 'rgba(255,255,255,0.52)' }} />
        ...
        <div className="relative z-10 w-full max-w-2xl flex flex-col items-center text-center pt-24">
          ...
          >
            {name ? `¡Listo, ${name}!` : '¡Llamada agendada!'}
          </h1>
          ...
          >
            Tu demo está confirmada. Estos son los próximos pasos para que la llamada sea un éxito.
          </p>
