...
type SectionKey = 'tracking' | 'spy' | 'production'

type Props = {
  section: SectionKey
  /** Optional mountain separator rendered above the section. */
  separator?: { image: string; position: string }
  /** Render the title and accent on the same line instead of stacked. */
  inlineTitle?: boolean
}

export function FeatureSection({ section, separator, inlineTitle = false }: Props) {
  const { t } = useLanguage()
  const data = t.sections[section]

  return (
    <>
      {separator && <MountainSeparator image={separator.image} position={separator.position} />}

      <section id={data.id} className="bg-white px-4 py-12 md:py-24">
        <div className="mx-auto max-w-6xl space-y-4 md:space-y-6">
          ...
          <AnimateIn className="text-center mb-8 md:mb-16">
            ...
            >
              <span style={{ fontSize: '0.65rem' }}>✦</span> {data.badge}
            </span>
...
function FeatureImage({ image, alt }: { image: string | null; alt: string }) {
  return (
    <div
      className="relative w-full h-[220px] md:h-[300px] rounded-xl flex items-center justify-center overflow-hidden"
      style={{
        background: '#fff',
    ...
    >
      {image ? (
        <Image src={image} alt={alt} fill sizes="(min-width: 768px) 60vw, 100vw" className="object-contain object-center" />
      ) : (
        <AppMockup />
      )}
    </div>
